...
// Static IP configuration for Node 2
...
void setup() {
  ...
  Serial.println("\n========================================");
  Serial.println("   NODE 2 - Bedroom Controller");
  Serial.println("========================================");
    ...
    if (server.hasArg("device") && (server.hasArg("state") || server.hasArg("action"))) {
      ...
      if (device == "fan") {
        if (stateStr == "on" || stateStr == "true") {
          ...
          fanState = false;
          success = true;
          message = "FAN 2 turned OFF";
          newState = false;
        } else if (stateStr == "toggle") {
          fanState = !fanState;
          digitalWrite(FAN_PIN, fanState ? LOW : HIGH);
          success = true;
          message = fanState ? "FAN 2 toggled ON" : "FAN 2 toggled OFF";
          newState = fanState;
        }
      } else if (device == "led") {
        if (stateStr == "on" || stateStr == "true") {
          ...
          ledState = true;
          success = true;
          message = "LED 2 turned ON";
          newState = true;
        } else if (stateStr == "off" || stateStr == "false") {
          digitalWrite(LED_PIN, HIGH);
          ledState = false;
          success = true;
          message = "LED 2 turned OFF";
          newState = false;
        } else if (stateStr == "toggle") {
          ledState = !ledState;
          digitalWrite(LED_PIN, ledState ? LOW : HIGH);
          success = true;
          message = ledState ? "LED 2 toggled ON" : "LED 2 toggled OFF";
          newState = ledState;
        }
      }
